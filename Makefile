...
IDIRS+=$(MNEMOSYNE_INCLUDES)/mtm/include $(MNEMOSYNE_INCLUDES)/pmalloc/include \
 $(MNEMOSYNE_INCLUDES)/mcore/include
LDIRS+=$(MNEMOSYNE_LIBS)/mtm $(MNEMOSYNE_LIBS)/pmalloc $(MNEMOSYNE_LIBS)/mcore #$(LIBITM_PATH)
LIBS+=-lmtm -lpmalloc -lmcore -lelf -lconfig -litm
CFLAGS += -fgnu-tm -march=native -DMNEMOSYNE -O2 -DNDEBUG  -fno-strict-aliasing -msse2 -mfpmath=sse -fno-schedule-insns -fno-schedule-insns2 -fno-tree-ch
CXXFLAGS += -fgnu-tm -march=native -DMNEMOSYNE -O2 -std=c++1y -DNDEBUG  -fno-strict-aliasing -msse2 -mfpmath=sse -fno-schedule-insns -fno-schedule-insns2 -fno-tree-ch
LD += -fgnu-tm -T $(MNEMOSYNE_PATH)/usermode/tool/linker/linker_script_persistent_segment_m64
# we can add additional release customization here
# e.g. link against different libraries, 
# define enviroment vars, etc.
endif

ifeq ($(BUILD),pronto-full)
IDIRS+=$(PRONTO_PATH)/src
LDIRS+=$(PRONTO_PATH)/src
LIBS+=-lsavitar -lpmem -luuid -lstdc++fs -lstdc++
CFLAGS+=-DPRONTO -fno-stack-protector -U__STRICT_ANSI__
CXXFLAGS+=-DPRONTO -fno-stack-protector -U__STRICT_ANSI__
# we can add additional release customization here
# e.g. link against different libraries, 
# define enviroment vars, etc.
endif

ifeq ($(BUILD),pronto-sync)
IDIRS+=$(PRONTO_PATH)/src
LDIRS+=$(PRONTO_PATH)/src
