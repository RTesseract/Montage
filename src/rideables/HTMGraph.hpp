#ifndef HTM_GRAPH_HPP
#define HTM_GRAPH_HPP
...
class HTMGraph : public RGraph, public Recoverable{
    ...
    public:
        ...
        class alignas(64) tVertex {
            public:
                ...
                tVertex(HTMGraph* ds_, int id, int lbl): ds(ds_) {
                    payload = ds->pnew<Vertex>(id, lbl);
                    this->id = id;
                }
                tVertex(HTMGraph* ds_, Vertex* p): ds(ds_) {
        ...
        struct alignas(64) VertexMeta {
            ...
            GlobalLock vertexLocks;// Transient locks for transient vertices
            uint32_t vertexSeqs;// Transient sequence numbers for transactional operations on vertices
        };

        HTMGraph(GlobalTestConfig* gtc) : Recoverable(gtc), gtc(gtc) {
...
class HTMGraphFactory : public RideableFactory{
    Rideable *build(GlobalTestConfig *gtc){
        return new HTMGraph<numVertices, meanEdgesPerVertex, vertexLoad>(gtc);
    }
};
